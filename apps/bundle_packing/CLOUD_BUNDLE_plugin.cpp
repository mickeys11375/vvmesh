...
bool CLOUD_BUNDLE_Plugin::set_rays_mva(const std::vector<RayBundle> &raybundles,
  ...
  std::vector<std::set<uint32_t>> vec_id_set(raybundles.size());
  ...
  for (int j = 0; j < raybundles.size(); ++j) {
    ...
    ifs.close();
    // process point id;
    std::set<uint32_t> &id_set = vec_id_set[j];
  ...
  for (int j = 0; j < vec_id_set.size(); ++j)
  {
    auto &id_set = vec_id_set[j];
    for (auto &pID : id_set) 
    {
      MVSA::Interface::Vertex::View view;
      view.imageID = j;  // image id
      view.confidence = 1.0;
      obj.vertices[pID].views.push_back(view);
    }
  }
